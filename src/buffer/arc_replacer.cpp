...
auto ArcReplacer::Evict() -> std::optional<frame_id_t> {
  ...
  if (mru_.size() >= mru_target_size_) {
    if (auto victim = try_evict_from(mru_, mru_ghost_, ArcStatus::MRU_GHOST)) {
      return victim;
    }
    return try_evict_from(mfu_, mfu_ghost_, ArcStatus::MFU_GHOST);
  }

  if (auto victim = try_evict_from(mfu_, mfu_ghost_, ArcStatus::MFU_GHOST)) {
    return victim;
  }
  return try_evict_from(mru_, mru_ghost_, ArcStatus::MRU_GHOST);
}
